...
//import jehep.misc.FontChooser.NwList;
...
public class PeakCurve implements Serializable  {
    ...
    //Y: m/z
    //Z: intensity
    public XYPointCollection SmoothData;
    public XYPointCollection newdata;
    //X: retention time
    //Y: intensity
    //XIC
    ...
    public void newDoBspline() {
//    	HashMap<Float, Float> rtMz = new HashMap();
//    	for (int aa=0; aa<PeakList.size();aa++) {
//    		if(rtMz.containsKey(PeakList.getXat(aa))) {
////    			rtMz.put(PeakList.getXat(aa),(rtMz.get(PeakList.getXat(aa)) +PeakList.getZat(aa))/2);
//    			rtMz.put(PeakList.getXat(aa)+0.00001f,PeakList.getZat(aa));
//    		}
//    		else {
//    			rtMz.put(PeakList.getXat(aa), PeakList.getZat(aa));
//    		}
//    		
//    	}
//    	double[] arrayA = new double[rtMz.size()];
//    	double[] arrayB = new double[rtMz.size()];
//    	Set sortSet = rtMz.keySet();
//    	Object[] sortarr = sortSet.toArray();
//    	Arrays.sort(sortarr);
//    	int ii=0;
//    	for(Object key:sortarr) {
//    		arrayA[ii]=Double.parseDouble(String.valueOf(key));
//    		arrayB[ii]=Double.parseDouble(String.valueOf(rtMz.get(key)));
//    		ii++;
//    	}
//    	double[] smoothY = new double[rtMz.size()];
//    	if (rtMz.size()>3) {
//    	try {
//      	
//    		LoessInterpolator regresson = new LoessInterpolator(0.5,4);
//			smoothY = regresson.smooth(arrayA, arrayB);	
//		} catch (Exception e) {
//			
...
//        	XYData pt = new XYData((float) arrayA[i], (float) smoothY[i]);
//            SmoothData.AddPoint(pt);
//        }
//    	}
//    	else {
...
/*----------------------------------------------cubic spline---------------------------------------------------------------------*/
    	XYPointCollection tmp = new XYPointCollection();	
    	HashMap<Float, Float> rtMz = new HashMap();
    	for (int aa=0; aa<PeakList.size();aa++) {
    		if(rtMz.containsKey(PeakList.getXat(aa))) {
//    			rtMz.put(PeakList.getXat(aa),(rtMz.get(PeakList.getXat(aa)) +PeakList.getZat(aa))/2);
    			rtMz.put(PeakList.getXat(aa)+0.0001f, PeakList.getZat(aa));
    		}
    		else {
    			rtMz.put(PeakList.getXat(aa), PeakList.getZat(aa));
    		}
    	...
    	int numPoints = (int) Math.max(RTWidth()*parameter.NoPeakPerMin, rtMz.size());
		...
		float range=(float) (arrayA[arrayA.length-1]-arrayA[0])/numPoints;
//    	double[]  smoothY = new double[numPoints];
        try {
        	
//        	LoessInterpolator regresson = new LoessInterpolator(0.5,2);
        	org.apache.commons.math3.analysis.interpolation.SplineInterpolator regresson = new org.apache.commons.math3.analysis.interpolation.SplineInterpolator();
			PolynomialSplineFunction f = regresson.interpolate(arrayA, arrayB);
//        	SmoothingCubicSpline fit = new SmoothingCubicSpline(arrayA, arrayB, 0.5);
			for(int i=0; i<numPoints; i++) {
//				smoothY[i]=f.value(arrayA[0]+range*i);
//				smoothY[i]=fit.evaluate(arrayA[0]+range*i);
				tmp.AddPoint((float) (arrayA[0]+range*i), (float) (f.value(arrayA[0]+range*i)));
//				tmp.AddPoint((float) (arrayA[0]+range*i), (float) (fit.evaluate(arrayA[0]+range*i)));
			}		
		} catch (Exception e) {
...
//            MzVar += (PeakList.get(j).getX() - TargetMz) * (PeakList.get(j).getX() - TargetMz);
    ...
    public void newsmoothing() {
    	...
    	double[] xValue=new double[PeakList.size()];
    	double[] yValue=new double[PeakList.size()];

    	
    	for(int i=0; i<PeakList.size(); i++) {
    		xValue[i]=PeakList.getXat(i);
    		yValue[i]=PeakList.getZat(i);
    	}
    	try {
			LoessInterpolator regresson = new LoessInterpolator(0.5,4);
			PolynomialSplineFunction f = regresson.interpolate(xValue, yValue);
			for(int i=0; i<numPoints; i++) {
				smoothY[i]=f.value(xValue[0]+range*i);
				tmp.AddPoint((float) (xValue[0]+range*i), (float) (f.value(xValue[0]+range*i)));
			}
		} catch (Exception e) {
